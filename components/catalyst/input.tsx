  ...
  return (
    <span
      className={clsx([
        ...
        // Background color + shadow applied to inset pseudo element, so shadow blends with border in light mode (only visible when not borderless)
        !borderless &&
          "before:absolute before:inset-px before:rounded-[calc(theme(borderRadius.lg)-1px)] before:bg-white before:shadow",
        // Background color is moved to control and shadow is removed in dark mode so hide `before` pseudo
        !borderless && "dark:before:hidden",
        // Focus ring (only visible when not borderless)
        !borderless &&
