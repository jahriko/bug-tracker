...
test("Should update an issue's status", async ({ page }) => {
  ...
  await page.waitForSelector('button[aria-label="Status"]:has-text("In Progress")')

  // Verify the status has been updated
  const statusElement = page.locator('button[aria-label="Status"]:has-text("In Progress")')
...
test("Should assign a user to an issue", async ({ page }) => {
  ...
  await page.waitForSelector('button[aria-label="Assigned to"]:has-text("John Doe")')

  // Verify the assignee has been updated
  const assigneeElement = page.locator('button[aria-label="Assigned to"]:has-text("John Doe")')
