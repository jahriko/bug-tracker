    ...
    async (tx) => {
      ...
      await tx.user.updateMany({
        where: {
          id: {
            in: (
              await tx.workspaceMember.findMany({ where: { workspaceId: workspace.id }, select: { userId: true } })
            ).map((m) => m.userId),
          },
        },
      ...
      console.log(`Existing members: ${existingMembers.map((m) => m.userId).join(", ")}`)

      if (existingMembers.length < 3) {
        console.warn(`Only ${existingMembers.length} of the selected admin IDs are valid workspace members`)
      }
      ...
      console.log(`Update result: ${updateResult.count} members updated to ADMIN`)

      // Verify the update
      const updatedAdmins = await tx.workspaceMember.findMany({
        where: { userId: { in: adminIds }, workspaceId: workspace.id, role: "ADMIN" },
      ...
      if (updatedAdmins.length < 3) {
        ...
        if (remainingMembers.length > 0) {
          const additionalUpdateResult = await tx.workspaceMember.updateMany({
            where: { userId: { in: remainingMembers.map((m) => m.userId) }, workspaceId: workspace.id },
            data: { role: "ADMIN" },
          })
          console.log(`Additional update result: ${additionalUpdateResult.count} more members updated to ADMIN`)
        }
      }
      ...
      for (const workspaceId of workspaceIds) {
        ...
        const projectIds = (await tx.project.findMany({ where: { workspaceId }, select: { id: true } })).map(
          (p) => p.id,
        )
        ...
        for (const projectId of projectIds) {
          ...
          await tx.projectMember.createMany({
            data: faker.helpers.arrayElements(users, memberCount).map((userId) => ({
              userId,
              projectId,
            })),
            skipDuplicates: true,
      ...
      for (const projectId of projectIds) {
        ...
        if (projectMemberIds.length === 0) {
          console.warn(`Project ${projectId} has no members. Skipping issue creation.`)
          continue
        }
      ...
      const issueIds = (
        await tx.issue.findMany({ where: { projectId: { in: projectIds } }, select: { id: true } })
      ).map((i) => i.id)
      ...
      const categoryIds = (await tx.discussionCategory.findMany({ select: { id: true } })).map((c) => c.id)
      for (const projectId of projectIds) {
        ...
        const discussionIds = (await tx.discussion.findMany({ where: { projectId }, select: { id: true } })).map(
          (d) => d.id,
        )
