...
type GroupedLabelActivityType = Extract<IssueActivityType[number], { issueActivity: "GroupedLabelActivity" }>
...
type LabelActivityType = Extract<IssueActivityType[number], { issueActivity: "LabelActivity" }>

const createNewLabelGroup = (activity: LabelActivityType): GroupedLabelActivityType => ({
...
const updateLabelGroup = (
  ...
  const removeIndex = arrayToRemoveFrom.findIndex((label) => label.name === labelName)
...
const shouldStartNewLabelGroup = (
  ...
  const timeDifference = new Date(activity.createdAt).getTime() - new Date(prevActivity.createdAt).getTime()
 ...
 */
const finalizeGroup = (result: IssueActivityType[number][], currentGroup: GroupedLabelActivityType | null) => {
  if (currentGroup) {
    if (currentGroup.addedLabels.length > 0) {
      result.push({ ...currentGroup, id: `group-add-${currentGroup.id}`, removedLabels: [] })
    }
    if (currentGroup.removedLabels.length > 0) {
      result.push({ ...currentGroup, id: `group-remove-${currentGroup.id}`, addedLabels: [] })
    }
  }
}
...
function ActivityFeed({ issue, activities, children }: ActivityFeedContextType & { children: ReactNode }) {
  ...
  return (
    <ActivityFeedContext.Provider value={{ issue, activities: processedLabelActivities }}>
        ...
        <ul className="space-y-6" role="list">
          <ActivityItem>
            <div className="relative flex h-6 w-6 flex-none items-center justify-center rounded-full bg-gray-50 ring-1 ring-gray-200">
              <Avatar alt={issue?.owner?.name ?? "User"} className="size-5" src={issue?.owner?.image ?? ""} />
            </div>
            <div className="flex gap-x-2">
              <div className="flex-auto py-0.5 text-xs leading-5 text-gray-500">
                <span className="font-medium text-gray-900">{issue?.owner?.name}</span> created this issue{" "}
              </div>
              <span>⋅</span>
              <time
          ...
          {processedLabelActivities.map((item, itemIdx) => (
            <ActivityItem itemIdx={itemIdx} key={item.id}>
              ...
              {item.issueActivity === "CommentActivity" && <CommentActivity comments={issue.comments} item={item} />}
            </ActivityItem>
          ))}
        </ul>
      </div>
...
function LabelActivity({
  ...
  item: Extract<IssueActivityType[number], { issueActivity: "LabelActivity" | "GroupedLabelActivity" }>
}) {
  ...
  } else if (item.issueActivity === "LabelActivity") {
    if (item.action === "add") {
...
function LabelBadge({ color, name }: { color: string; name: string }) {
  return (
    <span className="inline-flex items-center gap-x-1.5 rounded-full px-2 py-1 text-2xs font-medium text-gray-900 ring-1 ring-inset ring-gray-200">
      <div className={classNames(COLORS[color] || "bg-zinc-100", "flex-none rounded-full p-1")}>
