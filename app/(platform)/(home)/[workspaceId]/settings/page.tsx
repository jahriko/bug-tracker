...
export default async function SettingsPage({ params }: { params: { workspaceId: string } }) {
  ...
  return (
    <div>
      ...
      <dl className="mt-6 space-y-6 divide-y divide-gray-100 border-t border-gray-200 text-sm leading-6">
        <div className="pt-6 sm:flex">
          <dt className="font-medium text-gray-900 sm:w-64 sm:flex-none sm:pr-6">Project Visibility</dt>
          <dd className="mt-1 flex justify-between gap-x-6 sm:mt-0 sm:flex-auto">
            <div className="text-gray-900">{workspace.private ? "Private" : "Public"}</div>
            <Button plain>
              {workspace.private ? (
                <>
